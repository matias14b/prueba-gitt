...
Vue.component('portabilidad-datos-envio-chip', {
  ...
  computed: {
    ...
    localidadElegidaPerteneceALocalidades() {
      const nombreLocalidades = this.localidades.map(localidad => localidad.nombre);
      if(nombreLocalidades.includes(this.solicitudChip.envioChip.localidad.nombre)) {
        return true;
      } else {
        this.solicitudChip.envioChip.localidad.id = null;
        return false;
      }
    }
  },
